...
<div class="col-md-6">
    <select class="form-select @error('university') is-invalid @enderror" wire:model.lazy="university" id="university">
        <option hidden>@lang('Select your attended university') <b class="text-danger">*</b></option>
        @foreach ($universities as $key => $university)
            <option value="{{ $key }}">@lang('University of '){{ $university }}</option>
        @endforeach
    </select>
    @error('university')
        <span class="text-danger fw-light"><small>{{ $message }}</small></span>
